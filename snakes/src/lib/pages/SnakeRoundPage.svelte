<script lang="ts">
	...
	onMount(async () => {
		...
		socket.on('worker:timeout', ({ snakeId, type, kind, identity, latency, runId }) => {
			for (const worker of Object.values(workers)) {
				worker.workflows.delete(snakeId);
			}
			console.log(snakeId, 'task timeout', { type, kind, identity, latency, runId }, `http://localhost:8233/namespaces/default/workflows/${snakeId}/${runId}/history`);
			workers = workers;
		});
