...
const webSocketServer = {
	...
	configureServer(server: ViteDevServer) {
		...
		io.on('connection', (socket) => {
			...
			// Player UI -> Game
			socket.on('roundStart', async ({ snakes }: { snakes: Snake[] }) => {
				const game = temporal.workflow.getHandle(GAME_WORKFLOW_ID);
				try {
					console.log('roundStart', { snakes });
					await game.signal('roundStart', { snakes });
				} catch (err) {
			...
			socket.on('worker:timeout', ({ snakeId, type, kind, identity, runId, latency }) => {
				io.emit('worker:timeout', { snakeId, type, kind, latency, runId });
			});
		});
	}
};
